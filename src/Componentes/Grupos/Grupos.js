    ...
    return(
        props.users.length > 0 && <section className='grupos' style={{ background: props.corSecundaria }}>
            ...
            <div className='cardsPLayers' style={{ color: props.corPrimaria }}>
                {props.users.map(user => <Jogadores key={user.nome} nome={user.nome} time={user.time} selecao={user.selecao} cor={user.cor} style={{ border: props.corPrimaria }} corDeFundo={props.corSecundaria} />)}
            </div>
        </section>
    )
}
